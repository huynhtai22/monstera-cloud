            ...
            <div className="rounded-2xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-slate-800/40 backdrop-blur-sm p-6 shadow-sm">
                ...
                <div className="space-y-4">
                    {MOCK_TEAM.map(member => (
                        <div key={member.id} className="flex items-center justify-between p-4 rounded-xl border border-black/5 dark:border-white/5 bg-black/5 dark:bg-slate-900/40">
                            ...
                            <div className="flex items-center space-x-4">
                                <span className="text-xs font-medium px-2.5 py-1 rounded-md bg-black/10 dark:bg-slate-700/60 text-slate-600 dark:text-slate-300">
                                    {member.role}
                                </span>
                                {member.role !== 'Admin' && (
