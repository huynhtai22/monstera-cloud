...
export function SourceScopePanel({
  ...
  if (needsReconnect || !managerBadge) {
    return (
      <SourceReconnectBanner
        provider={provider}
        needsReconnect={needsReconnect || !managerBadge}
        onReconnect={onReconnect}
      />
    );
  }

  const count = summarizeAccountScope(provider, [{ id: managerBadge, label: managerBadge }], 1);
  return (
