...
import { hashTelemetryValue } from "./login-telemetry";
import { isApiKeyIpAllowed, pinHashForRequest } from "./api-key-security";
...
describe("versioned API-key pin salts", () => {
  ...
  it("keeps pre-versioning pins valid through the migration window", () => {
    ...
    process.env.API_KEY_PIN_SALT_VERSION = "v2";
    const legacy = hashTelemetryValue("192.0.2.44", "legacy-login-salt");
