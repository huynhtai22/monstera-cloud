...
export function generateReviewerMarkdown(pack: CertificationEvidencePack): string {
  ...
  if (pack.evidenceClass === "synthetic_fixture") {
    md += `> [!CAUTION]\n`;
    md += `> **Synthetic Fixture:** This evidence pack was produced from synthetic fixtures. It is strictly ineligible for pilot certification or production acceptance (Certification Eligible: NO ❌, Pilot Eligible: NO ❌) and cannot advance through live gates.\n\n`;
  }
  ...
  if (pack.operatorSignOff) {
    md += `### Authenticated Operator Sign-Off\n\n`;
    md += `- **Reviewer User ID:** \`${pack.operatorSignOff.reviewerUserId}\`\n`;
    md += `- **Reviewer Role:** \`${pack.operatorSignOff.reviewerRole}\`\n`;
    md += `- **Signed At:** \`${pack.operatorSignOff.signedAt}\`\n`;
    md += `- **Evidence Pack Hash:** \`${pack.operatorSignOff.evidencePackHash}\`\n`;
    if (pack.operatorSignOff.comments) {
      md += `- **Comments:** ${pack.operatorSignOff.comments}\n`;
    }
    md += `\n`;
  }

  if (pack.ownerAttestation) {
    md += `### Workspace Owner Attestation\n\n`;
    md += `- **Owner User ID:** \`${pack.ownerAttestation.ownerUserId}\`\n`;
    md += `- **Owner Role:** \`${pack.ownerAttestation.ownerRole}\`\n`;
    md += `- **Attested At:** \`${pack.ownerAttestation.attestedAt}\`\n`;
    if (pack.ownerAttestation.comments) {
      md += `- **Comments:** ${pack.ownerAttestation.comments}\n`;
    }
    md += `_Note: Workspace owner attestation does not award PILOT_CERTIFIED status._\n\n`;
  }
