...
export async function reportingDataset(tx: ScopedTransaction, workspaceId: string, clientId: string, window: ReportingWindow, providerScope?: string[]) {
  ...
  const [rows, contexts, sources] = await Promise.all([
    tx.campaignMetric.findMany({ where: { workspaceId, connection, ...(scope ? { platform: { in: scope } } : {}), date: { gte: new Date(`${window.start}T00:00:00Z`), lte: new Date(`${window.end}T23:59:59.999Z`) } }, orderBy: { id: "asc" }, take: REPORT_DATASET_CAP + 1 }),
...
export async function retrieveClientDelivery(input: WarehouseQueryInput & { clientId: string }, destination: "google_sheets" | "looker_studio" | null, actorId: string) {
  ...
  return prisma.$transaction(async tx => {
    ...
    const scopedResultRows = scope ? result.rows.filter((row: { platform?: string }) => scopeSet.has(String(row.platform ?? ""))) : result.rows;
    let receiptId: string | null = null;
    if (destination && !snapshot.limited && snapshot.rowCount > 0 && snapshot.dataThroughDate && !result.pagination.hasMore &&
        !input.cursor && !input.offset && !input.platforms?.length && !input.accountIds?.length && !input.connectionId && !input.campaignId && scopedResultRows.length === snapshot.rowCount) {
