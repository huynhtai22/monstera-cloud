...
async function syncCampaigns(input: {
  ...
  try {
    const pages = await shopeeAdsClient.getAllProductLevelCampaignPages(input.api);
    ...
    try {
      settings = ids.length ? await shopeeAdsClient.getProductLevelCampaignSettingPages(input.api, ids) : [];
    } catch (error) {
