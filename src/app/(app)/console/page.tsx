...
export default function ConsolePage() {
    ...
    /* #10 — Updated disconnect copy to user-facing language */
    async function disconnectSource(connectionId: string, displayName: string) {
        const ok = window.confirm(
            `Disconnect "${displayName}"?\n\nThis will stop all syncs from this source and remove its saved credentials. You can reconnect later.`
        );
        if (!ok) return;
    ...
    useEffect(() => {
        trackOnce("mc_console_session", "console_visit", { path: "/console" });
    }, []);
                ...
                <div>
                    {isLoading ? (
                        ...
                        <>
                            ...
                            <p className="text-gray-500 dark:text-gray-400 max-w-2xl text-base">
                                Pick a platform to connect. Your first dashboard is ~3 minutes away — choose{" "}
