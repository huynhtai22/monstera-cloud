...
docker exec -i "${container}" psql -U postgres -d monstera_rls < scripts/tenant-rls-policy.sql >/dev/null
...
printf '{"runId":"%s","environment":"local-disposable","result":"pass","sameTenantRows":1,"unscopedRows":0,"crossTenantInsert":"rejected"}\n' "${run_id}"
