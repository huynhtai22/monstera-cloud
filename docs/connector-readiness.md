...
| **Google Ads** | `google_ads` | Certified | `GOOGLE_ADS_CLIENT_ID`, `GOOGLE_ADS_CLIENT_SECRET`, `GOOGLE_ADS_DEVELOPER_TOKEN` | Verified (Session + Connection RBAC) | ✅ **Basic Access approved 2026-08-25 — live account validation pending** (see `docs/google-ads-basic-access.md`) | Customer client search, report sync to `CampaignMetric`, refresh token lifecycle. Developer token is env-only (never persisted on connections); provider-echoed errors are scrubbed of the token value; unit suite in `src/lib/google-ads.test.ts`. |
