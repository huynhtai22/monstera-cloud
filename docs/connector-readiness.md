...
| **Google Ads** | `google_ads` | Certified | `GOOGLE_ADS_CLIENT_ID`, `GOOGLE_ADS_CLIENT_SECRET`, `GOOGLE_ADS_DEVELOPER_TOKEN` | Verified (Session + Connection RBAC) | ⚠️ Pending Sandbox Keys | Customer client search, report sync to `CampaignMetric`, refresh token lifecycle. |
